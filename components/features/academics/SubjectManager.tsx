...
// Subject is a 2-field master entity (name, code) — a full separate "new"/"edit" route per
// admission-form.tsx's pattern would be disproportionate; one inline create form + editable
// table covers create/update/delete in a single client component instead.
export function SubjectManager({ items, canManage }: SubjectManagerProps) {
  ...
  async function handleToggleActive(subject: SubjectDTO) {
    setEditingId(subject.id);
    setError(null);
    try {
      const result = await updateSubjectAction(subject.id, { isActive: !subject.isActive });
      if (!result.success) {
        setError(result.error.message);
        return;
      }
      ...
      {canManage && (
        <div className="flex flex-wrap items-end gap-3 rounded-xl border border-zinc-200 bg-white p-4">
          <div className="flex flex-col gap-1">
            <label htmlFor="subject-name" className="text-xs font-medium text-zinc-500">
              Name
            </label>
            <input
              id="subject-name"
          ...
          <div className="flex flex-col gap-1">
            <label htmlFor="subject-code" className="text-xs font-medium text-zinc-500">
              Code
            </label>
            <input
              id="subject-code"
      ...
      <div className="overflow-hidden rounded-xl border border-zinc-200">
        <table className="min-w-full divide-y divide-zinc-200 text-sm">
          ...
          <tbody className="divide-y divide-zinc-100 bg-white">
            {items.map((subject) => (
              <tr key={subject.id}>
                <td className="px-4 py-2 text-zinc-900">{subject.name}</td>
                <td className="px-4 py-2 text-zinc-700">{subject.code}</td>
                <td className="px-4 py-2 text-zinc-700">{subject.isActive ? "Active" : "Inactive"}</td>
                {canManage && (
        ...
        {items.length === 0 && <p className="p-4 text-sm text-zinc-500">No subjects yet.</p>}
      </div>
    </div>
  );
}
