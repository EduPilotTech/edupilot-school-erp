...
import { bulkMarkHostelAttendanceAction } from "@/app/hostel/actions";

export interface HostelAttendanceRow {
  studentId: string;
...
interface HostelAttendanceMarkerProps {
  roomId: string;
  academicSessionId: string;
  date: string;
  session: string;
  rows: HostelAttendanceRow[];
}

const STATUS_OPTIONS = ["PRESENT", "ABSENT", "ON_LEAVE"];

// One room/date/session, every student in the room marked in a single submit — mirrors
// TransportAttendanceMarker's own shape. Marking ABSENT triggers a parent notification
// server-side (see bulk-mark-hostel-attendance.service.ts), not handled here.
export function HostelAttendanceMarker({ roomId, academicSessionId, date, session, rows }: HostelAttendanceMarkerProps) {
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const result = await bulkMarkHostelAttendanceAction({
        roomId,
        academicSessionId,
        date,
        session,
  ...
  if (rows.length === 0) {
    return <p className="text-sm text-zinc-500">No students assigned to this room yet.</p>;
  }
