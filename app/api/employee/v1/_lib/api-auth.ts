...
// The Employee Portal counterpart to app/api/parent/v1/_lib/api-auth.ts's own precedent —
// deliberately NOT a reuse of requireAuthContext()/requireCurrentUser(), which redirect to
// /login on failure (a browser-UX concern, fine for Server Components, wrong for a JSON API). A
// REST API meant for a future Android/iOS Employee Portal client (same "one application layer,
// two delivery mechanisms" decision Phase 9 already made for Parents) must return a clean 401/403
// JSON response instead of an HTTP redirect to an HTML login page. Reuses the exact same
// `resolvePermissions` query getAuthorizationContext() does — only the failure behavior differs.
...
// Shared error -> Response mapping for every app/api/employee/v1/**/route.ts handler — kept
// generic on the base NotFoundError/BusinessRuleError/ValidationError classes only, exactly
// matching app/api/parent/v1/_lib/api-auth.ts's own level of granularity (every HR/Payroll-
// specific error already extends one of these three bases, so no named-code mapping is needed for
// a first mobile API surface). A genuinely unexpected error is rethrown, never swallowed into a
// client-facing 400 that could leak internal detail (docs/SECURITY_GUIDELINES.md §10) — it
// surfaces as Next.js's own unhandled-route-error 500.
