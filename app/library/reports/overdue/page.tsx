...
import { getOverdueReport } from "@/modules/library/application/get-circulation-reports.service";
...
export default async function OverdueReportPage({ searchParams }: PageProps) {
  ...
  const rows = libraryId ? await getOverdueReport(authContext.tenantId, libraryId) : [];

  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      <h1 className="text-2xl font-semibold text-zinc-900">Overdue Report</h1>
      <p className="mt-1 text-sm text-zinc-500">Still-open issues past their due date, as of today.</p>
        ...
        <table className="min-w-full divide-y divide-zinc-200 text-sm">
          <thead className="bg-zinc-50">
            <tr>
              ...
              <th className="px-4 py-2 text-left font-medium text-zinc-500">Days Overdue</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-100 bg-white">
            {rows.map((row) => (
              <tr key={row.bookIssueId}>
                ...
                <td className="px-4 py-2 text-red-700">{row.daysOverdue}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {rows.length === 0 && <p className="p-4 text-sm text-zinc-500">No overdue books.</p>}
      </div>
    </main>
  );
}
