...
import { getReturnReport } from "@/modules/library/application/get-circulation-reports.service";

interface PageProps {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}

export default async function ReturnReportPage({ searchParams }: PageProps) {
  ...
  const rows = libraryId ? await getReturnReport(authContext.tenantId, libraryId) : [];

  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      <h1 className="text-2xl font-semibold text-zinc-900">Return Report</h1>
      <p className="mt-1 text-sm text-zinc-500">Every book returned in this library.</p>
      ...
      <div className="mt-6 overflow-hidden rounded-xl border border-zinc-200">
        ...
        {rows.length === 0 && <p className="p-4 text-sm text-zinc-500">No returns recorded.</p>}
      </div>
    </main>
  );
}
