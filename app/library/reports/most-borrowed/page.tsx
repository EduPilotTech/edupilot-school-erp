...
import { getMostBorrowedBooksReport } from "@/modules/library/application/get-fine-and-activity-reports.service";

interface PageProps {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}

export default async function MostBorrowedBooksReportPage({ searchParams }: PageProps) {
  ...
  const rows = libraryId ? await getMostBorrowedBooksReport(authContext.tenantId, libraryId) : [];

  return (
    <main className="mx-auto max-w-3xl px-6 py-10">
      <h1 className="text-2xl font-semibold text-zinc-900">Most Borrowed Books</h1>
      <p className="mt-1 text-sm text-zinc-500">Top 20 titles by issue count.</p>
        ...
        <table className="min-w-full divide-y divide-zinc-200 text-sm">
          <thead className="bg-zinc-50">
            <tr>
              ...
              <th className="px-4 py-2 text-left font-medium text-zinc-500">Times Borrowed</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-100 bg-white">
            {rows.map((row, index) => (
              <tr key={row.bookId}>
                ...
                <td className="px-4 py-2 text-zinc-700">{row.borrowCount}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {rows.length === 0 && <p className="p-4 text-sm text-zinc-500">No circulation history yet.</p>}
      </div>
    </main>
  );
}
