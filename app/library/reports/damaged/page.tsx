...
import { getDamagedBooksReport } from "@/modules/library/application/get-circulation-reports.service";

interface PageProps {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}

export default async function DamagedBooksReportPage({ searchParams }: PageProps) {
  ...
  const rows = libraryId ? await getDamagedBooksReport(authContext.tenantId, libraryId) : [];

  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      <h1 className="text-2xl font-semibold text-zinc-900">Damaged Books</h1>
        ...
        <table className="min-w-full divide-y divide-zinc-200 text-sm">
          <thead className="bg-zinc-50">
            <tr>
              ...
              <th className="px-4 py-2 text-left font-medium text-zinc-500">Reported On</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-100 bg-white">
            {rows.map((row) => (
              <tr key={row.bookIssueId}>
                ...
                <td className="px-4 py-2 text-zinc-700">{row.returnDate ?? "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {rows.length === 0 && <p className="p-4 text-sm text-zinc-500">No damaged books.</p>}
      </div>
    </main>
  );
}
