...
import { getLostBooksReport } from "@/modules/library/application/get-circulation-reports.service";

interface PageProps {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}

export default async function LostBooksReportPage({ searchParams }: PageProps) {
  ...
  const rows = libraryId ? await getLostBooksReport(authContext.tenantId, libraryId) : [];

  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      <h1 className="text-2xl font-semibold text-zinc-900">Lost Books</h1>
      ...
      <div className="mt-6 overflow-hidden rounded-xl border border-zinc-200">
        ...
        {rows.length === 0 && <p className="p-4 text-sm text-zinc-500">No lost books.</p>}
      </div>
    </main>
  );
}
