...
import { getCategoryWiseExpenseReport } from "@/modules/finance/application/get-category-wise-expense-report.service";
import { listActiveAcademicSessions } from "@/modules/academics/application/list-active-academic-sessions.service";
import { CategoryWiseExpenseReportView } from "@/components/features/finance/CategoryWiseExpenseReportView";

interface CategoryWiseExpenseReportPageProps {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}

// The symmetric counterpart of app/finance/reports/category-wise-income/page.tsx.
export default async function CategoryWiseExpenseReportPage({ searchParams }: CategoryWiseExpenseReportPageProps) {
  ...
  const report = await getCategoryWiseExpenseReport(authContext.tenantId, authContext.schoolId, {
    academicSessionId,
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      ...
      <h1 className="mt-2 text-2xl font-semibold text-zinc-900">Category-wise Expense Report</h1>
      <p className="mt-1 text-sm text-zinc-500">Expense totals grouped by expense category, with entry counts.</p>
      ...
      <div className="mt-6">
        <CategoryWiseExpenseReportView report={report} fileName="category-wise-expense-report" />
      </div>
    </main>
  );
}
