...
import { promoteStudents } from "@/modules/students/application/promote-students.service";
import type { PromotionResultDTO } from "@/modules/students/application/dto/promote-students.dto";

export type ActionResult<T> =
  | { success: true; data: T }
  | { success: false; error: { code: string; message: string } };

function translatePromotionError(error: unknown): ActionResult<never> {
...
export async function promoteStudentsAction(input: unknown): Promise<ActionResult<PromotionResultDTO[]>> {
  const authContext = await requireAuthContext();
  await requirePermission("student.promote");

  try {
    const results = await promoteStudents(input, {
      tenantId: authContext.tenantId,
      actingUserId: authContext.userId,
    });
    return { success: true, data: results };
  } catch (error) {
    return translatePromotionError(error);
  }
}
